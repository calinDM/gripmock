...
func prepareProtos(protoDirs []string) (result, includeDirs []string) {
	protogenDir, err := filepath.Abs(protogen)
	if err != nil {
		log.Fatal("Fail on get abs path ", err)
	}
	for _, proto := range protoDirs {
		includeDirs = append(includeDirs, filepath.Join(protogenDir, filepath.Base(proto)))
		filepath.WalkDir(proto, func(path string, d os.DirEntry, err error) error {
			...
			if strings.HasSuffix(path, ".proto") {
				...
				dir := filepath.Join(filepath.Base(proto), filepath.Dir(relativePath))
				...
				if syntaxLineNum != -1 {
					goPackageString := fmt.Sprintf("option go_package = \"grpcmock/protogen/%s\";", dir)
